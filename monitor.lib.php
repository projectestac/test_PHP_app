<?php
/*
 * Functions needed for stats collection
 * 
 */
 
//auxiliary function to get content from an URL:
...
/***********************
 * Function to return distinct users in DB last 30 min
 * 
 * @param dbo 		Database Object
 * @param dbname 	Database name
 * @param tblprefix	Table prefix of users table
 * @param dbtype	Database type. Values pdomysql, oracle
 * 
 * @return cacti formated string
 * 
 */
   
function moodle_session_DB(&$dbo,$dbname,$tblprefix,$dbtype="pdomysql",$file2open='')
{
	if ($dbtype=="pdomysql")
	{
		$sql = "SELECT DISTINCT username FROM {$dbname}.{$tblprefix}user WHERE lastaccess >= (UNIX_TIMESTAMP() - 1800)";
		$result = $dbo->query($sql);
		return "sessions_db:".$result->rowCount();
		/*option2 
		* $sql = "SELECT COUNT(id) FROM {$dbname}.{$tblprefix}user WHERE lastaccess >= (UNIX_TIMESTAMP() - 1800)";
		* $query = $dbo->prepare($sql);
        * $query->execute();
        * return "sessions_db:" . $query->fetch()[0];
		*/
	}
	if ($dbtype=="oracle")
	{
		...
		$service = $dbname;
		$sql = 'SELECT serviceDB, activedId as id
		...
		foreach ($results as $row) {
			$schools[$row->id] = strtoupper($row->serviceDB);
			//Get the active users of each school:
			$con = get_dbconnection($service, $row->id, $row->serviceDB);
			if ($con) {
                $sql = 'SELECT count(username) FROM ' . $agora['moodle2']['prefix'] . 'user WHERE lastaccess >= ((sysdate - date \'1970-01-01\') * 86400 - '.$timediff.')';
                ...
                while (oci_fetch($stmt)) {
                	...
                	$usu_tmp += $result;
                	$monitor[$schools[$row->id]] += $result;
				}
				disconnect_moodle($con);
			}
		}
		ksort($monitor,SORT_NATURAL);
		foreach ($monitor as $db => $numusers){
			$return .= "$db:$numusers ";
		}
		$return .= "Total:$usu_tmp";
		//Consolidate result:
...
/**********************+
 * Function to control last execution
 * 
 * @return exec_mode
 **********************/
function last_exec($file2open, $file2openLK, $current_date, $interval)
{
	global $phpbin, $scriptname;
	
	if (file_exists($file2openLK)){
        $fp = fopen($file2openLK,"r+");
        $exec_date = fgets($fp);
        if ($current_date - $exec_date < $interval){
                fclose ($fp);
                //Read last execution result
...
# ============================================================================
# This is a script extraction  to retrieve information from a MySQL server for 
# input to a Cacti graphing process.  It is hosted at
# http://code.google.com/p/mysql-cacti-templates/.
function mysql_php_monitor(){
	
	global $debug, $mysql_user, $mysql_pass, $heartbeat, $cache_dir, $poll_time,
          $chk_options, $mysql_port, $mysql_ssl, $mysql_host, $CFG;
		  
//Control for connect thru cacti script
//if ( !array_key_exists('SCRIPT_FILENAME', $_SERVER) || basename(__FILE__) == basename($_SERVER['SCRIPT_FILENAME']) ) {

/*
 * En el fitxer de crida cal afegir:
 * 
   require_once ("/path/to/config-moodle2.php");

  //DBconnection:*/
$mysql_host = $CFG->dbhost;
$mysql_user = $CFG->dbuser;
$mysql_pass = $CFG->dbpass;
$mysql_port = 3306;
...
$debug_log = "$CFG->dataroot/debug_mysql.log"; # If $debug_log is a filename, it'll be used.
...
if (!isset($called_by_script_server)) {
   ...
   foreach ( explode(' ', $result) as $item ) {
   	  if (empty($wanted[0])) {
                $output[] = $item;
      } else if ( in_array(substr($item, 0, 2), $wanted) ) {
         $output[] = $item;
      }
   }
...
function ss_get_mysql_stats( $options ) {
   ...
   global $debug, $mysql_user, $mysql_pass, $heartbeat, $cache_dir, $poll_time,
          $chk_options, $mysql_port, $mysql_ssl, $mysql_host;
   ...
   $sanitized_host
       = str_replace(array(":", "/"), array("", "_"), $host);
...
function moodle_session_files($multi=false,$agoraf=false){
	...
	if (!$multi){
		ob_start();
		$total_sess = @system("find $path_sess/ -name \"$prefix_sess*\" -mmin -30 -ls |wc -l");
		$php_sess = @system("find $path_sess/ -name \"$prefix_sess*\" -size +1024c -mmin -30 -ls |wc -l");
		ob_end_clean();
		$result = "sessions_t:$total_sess sessions_a:$php_sess";
		$fp = fopen($php_tmp,'w');
        fwrite($fp,$result);
        fclose($fp);
	} else {
		$dir = scandir($php_temp_dir);
        foreach ($dir as $file){
        	if (preg_match("/phpfiles.log$/",$file)){
        		$line = file_get_contents($php_temp_dir.$file);
        		$arr_line = explode(" ",$line);
        		$total_sess += substr($arr_line[0],11);
        		$php_sess += substr($arr_line[1],11);
			}
		}
        $result =  "sessions_t:$total_sess sessions_a:$php_sess";
	}
